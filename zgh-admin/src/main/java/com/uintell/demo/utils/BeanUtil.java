...
public final class BeanUtil {
	 ...
	 * 通过反射获取值
	 */
	/*public static Object getValueFromField2(Field field, String name, Object value) {
		Object temp = value;
		Object object = value;
		String type = field.getType().getSimpleName();
		
		if (temp==null && ("Short,short,int,Integer,Long,long,Double,double,Float,float".contains(type))) {
			temp = "0";
		}
		
		if ("short".equalsIgnoreCase(type)) { // Short
			object = Short.valueOf(temp.toString());
		} else if ("char".equals(type) || "Character".equals(type)) { // Character
			if (!BeanUtil.isEmpty(temp.toString())){
				object = temp.toString().charAt(0);
			}
		} else if ("int".equals(type) || "Integer".equals(type)) { // Integer
			object = Integer.valueOf(temp.toString());
		} else if ("long".equalsIgnoreCase(type)) { // Long
			object = Long.valueOf(temp.toString());
		} else if ("byte".equalsIgnoreCase(type)) { // Byte
			object = Byte.valueOf(temp.toString());
		} else if ("float".equalsIgnoreCase(type)) { // Float
			object = Float.valueOf(temp.toString());
		} else if ("double".equalsIgnoreCase(type)) { // Double
			object = Double.valueOf(temp.toString());
		} else if ("boolean".equalsIgnoreCase(type)) { // Boolean
			if(StringUtils.equals(temp.toString(), "1")){//转换1/0
				temp = "true";
			}
			object = Boolean.valueOf(temp.toString());
		} else if ("Date".equals(type)) { // Date
			object = BeanUtil.parse(temp.toString());
		} else if ("byte[]".equalsIgnoreCase(type)) { // byte[]
			object = ConvertUtil.string2Bytes(temp.toString());
		} 
		return object;
	}*/
	 ...
	 * @param 需要转换的结果类型
	 */
	public static <T> T convertMap2Bean2(Map<String, Object> bean, Class<T> clz) {
		if (bean == null) {
			return null;
		}
		...
		try {
			...
			for (Field field : fields) {
				...
				boolean visiable = field.isAccessible();
				field.setAccessible(Boolean.TRUE);
				Object object = BeanUtil.getValueFromField(field, key, value.toString());
				field.set(t, object);
				field.setAccessible(visiable);
			}
			return t;
